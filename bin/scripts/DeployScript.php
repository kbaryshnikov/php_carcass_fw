...
class DeployScript extends Controller {
    ...
    protected function buildDistArchive(Corelib\Hash $Args) {
        ...
        $this->Response->writeLn('> Distrubution destination archive template: [' . $target_tgz_file_basename. '.${revisionNumber}.tgz]');
        $this->Response->writeLn('> Building the distribution archive in temporary file [' . $dist_tarball_file . ']...');
