<?php
    ...
    // fixxxer: replaced FIRELOGGER_* constants with FireLogger constants/static vars; moved global-space code to FireLoggerInitializer.
    ...
    class FireLogger {
        ...
        public static
            $NO_EXCEPTION_HANDLER = false,
            $NO_ERROR_HANDLER = false,
            $NO_DEFAULT_LOGGER = false,
            $NO_OUTPUT_HANDLER = false,
            $NO_PASSWORD_CHECK = false,
            $PASSWORD = NULL,
            $NO_VERSION_CHECK = true,
            $NO_ERROR_FILTERING = false;
        ...
        //------------------------------------------------------------------------------------------------------
        private function pickle($o, $maxLevel=FireLogger::MAX_PICKLE_DEPTH) {
            if ($maxLevel==0) {
        ...
        function log(/*level, fmt, obj1, obj2, ...*/) {
            ...
            if (count($args)>0 && is_object($args[0]) && is_a($args[0], 'Exception')) { // is_object check prevents http://pear.php.net/bugs/bug.php?id=2975
                ...
                $item['template'] = '['.get_class($e).'] '.$e->getMessage();
                $item['code'] = $e->getCode();
        ...
        static function firelogger_error_handler($errno, $errstr, $errfile, $errline, $errcontext) {
            if (FireLogger::$NO_ERROR_FILTERING) { // !defined('FIRELOGGER _NO_ERROR_FILTERING')) {
        ...
        static function handler($buffer) {
            ...
            /*
            ### fixxxer> removed: eats all the memory
    ...
    class FireLoggerInitializer {
        public static function init() {
    
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // decide if firelogger should be enabled
            if (!FireLogger::$NO_VERSION_CHECK) {
                if (!isset($_SERVER['HTTP_X_FIRELOGGER'])) {
                    ...
                    if (FireLogger::$clientVersion!=FireLogger::$recommendedClientVersion) {
                        trigger_error("FireLogger for PHP (v".FireLogger::VERSION.") works best with FireLogger extension of version ".FireLogger::$recommendedClientVersion.". You are currently using extension v".FireLogger::$clientVersion.". Please upgrade your Firefox extension: http://firelogger.binaryage.com/php");
                    }
                }
            }
            ...
            if (!FireLogger::$NO_PASSWORD_CHECK && !empty(FireLogger::$PASSWORD) && FireLogger::$enabled) {
                if (isset($_SERVER['HTTP_X_FIRELOGGERAUTH'])) {
                    $clientHash = $_SERVER['HTTP_X_FIRELOGGERAUTH'];
                    $serverHash = md5("#FireLoggerPassword#".FireLogger::$PASSWORD."#");
            ...
            if (!FireLogger::$NO_EXCEPTION_HANDLER) {
            ...
            if (!FireLogger::$NO_ERROR_HANDLER) {
